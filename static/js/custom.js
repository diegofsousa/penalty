...
function add_user(idUser, idHouse) {
    $.ajax({
        ...
        success : function(json) {
            console.log(idUser, idHouse);
            if (json != false) {
                console.log("Resultado do processamento: "+json);
                parent.window.document.location.href = '';
            } else {
                console.log("Resultado do processamento: "+json);
                alert("Usuário não encontrado, verifique se digitou o username correto");
            }            
        },
...
function remove_comment(idComent) {
    console.log("reconheceu função");
    $.ajax({
        url : "/ambiente/remove_comentario/",
        type : "POST",
        data : { 
            id : idComent
             },
